...
bool checkCollision(const Eigen::Vector2d &position, double L, double W, const std::vector<amp::Polygon> &obstacles, double padding = 0.2)
{
...
amp::KinoPath MyKinoRRT::plan(const amp::KinodynamicProblem2D &problem, amp::DynamicAgent &agent)
{
    ...
    // Variables
    int control_sample_number = 110;
    double delta_t = 0.01;
    double step_size = 0.05;
    double max_iterations = 600000;
    ...
    if (!solutionNotFound)
    {
       ...
       // std::cout << "before" << std::endl;
        ...
        if (goalFound)
        {
            ...
            int max_iterations = 5000000; // Set a max iteration limit
